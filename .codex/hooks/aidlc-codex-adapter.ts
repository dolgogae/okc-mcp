...
//     hookSpecificOutput.updatedInput + permissionDecision:"allow" so every
...
//     When the shared hook allows the stop with empty stdout, the shim emits
//     `{}` because Codex requires successful Stop hooks to return JSON.
//   - log-subagent: the shared hook is advisory and normally emits nothing;
//     the shim emits `{}` because successful SubagentStop hooks also require
//     JSON on Codex.
...
const JSON_REQUIRED_SUCCESS_TARGETS = new Set([
  "continue-workflow",
  "log-subagent",
]);

function normalizeCodexHookOutput(target: string, stdout: string, code: number): string {
  if (code === 0 && stdout.trim().length === 0 && JSON_REQUIRED_SUCCESS_TARGETS.has(target)) {
    return "{}\n";
  }
  return stdout;
}
...
if (!process.stdin.isTTY) {
  try {
    ...
    const stdout = normalizeCodexHookOutput(target, "", 0);
...
if (!bypassReplay) {
  try {
    ...
    const stdout = normalizePreToolUseOutput(
      normalizeCodexHookOutput(target, replay.stdout, replay.code),
    );
    if (stdout) process.stdout.write(stdout);
...
// Codex requires every PreToolUse updatedInput rewrite to carry an explicit
// allow decision. Shared hooks predate that requirement, so normalize their
// otherwise-valid output at this Codex-only boundary.
function normalizePreToolUseOutput(stdout: string): string {
  try {
    ...
    // Empty or non-JSON advisory output passes through untouched.
  }
  return stdout;
}
...
switch (target) {
  ...
  case "continue-workflow": {
    // Pass stdin verbatim and preserve a blocking response. The shared hook's
    // successful allow path is intentionally silent, so normalize that path
    // to valid empty JSON for Codex's Stop output contract.
    const r = runCore("aidlc-continue-workflow.ts", rawInput);
    const stdout = normalizeCodexHookOutput(target, r.stdout, r.code);
    persistResponse(stdout, r.code);
    if (stdout) process.stdout.write(stdout);
    return r.code;
  }
  ...
  case "deliver-stage-rules": {
    ...
    // bundle to message/items. Add Codex's explicit allow decision when that
    // shared hook returns an updatedInput rewrite.
    const r = runCoreWithStderr("aidlc-deliver-stage-rules.ts", rawInput);
    const answeredCode = r.code === 2 ? 2 : 0;
    const stdout = normalizePreToolUseOutput(r.stdout);
